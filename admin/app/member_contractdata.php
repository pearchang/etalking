...
//      $_POST['note'] .= "\n原合約:" . $_POST['sn'];
      ...
      {
        $bid = $r['id'];
        unset ($r['id'], $r['creator'], $r['modifier'], $r['rank'], $r['cdate'], $r['mdate']);
        $r['contract_id'] = $new_id;
        $rs2->insert('member_contract_bill', $r);
        $r['new_id'] = $rs2->last_id;
        $new_bill[$bid] = $r;
        //echo $rs2->last_sql;
      }
      fn_modify($id);
...
function fn_modify($id)
{
  ...
  foreach ($_POST['bill_sn'] as $idx => $sn)
  {
    ...
    else
    {
      $s[] = "'$sn'";
      // check paid
      $sql = "SELECT * FROM member_contract_bill WHERE sn = '$sn' AND contract_id = $id";
      $rs2->query($sql);
      $r = $rs2->fetch();
      if ($r['paid'] > 0)
        continue;
      if ($new_id)
      {
        $v['sn'] = $sn = getSerialNumber(SN_BILL);
        $s[] = "'$sn'";
        $v['check'] = 0;
      }
