	...
	function report_consultant_salary($data){
		...
		if($rs2->count>0){
          ...
          $class_id = $row['id'];
					$name = $row['first_name'].' '.$row['last_name'];
					if(!empty($row['chi_name'])) $name.= ' /'.$row['chi_name'];
					$consultant[$row['consultant_id']]['A'] = '';
          $consultant[$row['consultant_id']]['B'] = $name;
          $consultant[$row['consultant_id']]['C'] += ($row['type']==10 ? 0 : $row['hour']);
          $consultant[$row['consultant_id']]['D'] += ($row['type']==10 ? 0 : $row['salary']);
          $consultant[$row['consultant_id']]['E'] += ($row['type']!=10 ? 0 : $row['hour']);
          $consultant[$row['consultant_id']]['F'] += ($row['type']!=10 ? 0 : $row['wage']);
          $consultant[$row['consultant_id']]['G'] += ($row['reward']>=0 ? $row['reward'] : 0);
          $consultant[$row['consultant_id']]['H'] += ($row['reward']<=0 ? $row['reward'] : 0);
          $consultant[$row['consultant_id']]['I'] += ($row['type']==10 ? ($row['hour']*$row['wage'])+$row['reward'] : $row['salary']+$row['reward']);
			}
			foreach ($consultant as $values)
      {
        foreach($values as $j => $val){
          if($j=='C' || $j=='D' ||$j=='E' || $j=='F' || $j=='G' || $j=='H' || $j=='I')
            $objPHPExcel->getActiveSheet()->setCellValueExplicit( $j.$i, $val,PHPExcel_Cell_DataType::TYPE_NUMERIC);
