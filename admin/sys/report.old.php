<?php
	...
	function report_student_point($data){
		...
		$objPHPExcel->createSheet(1)->setTitle("退點點數明細");
		...
		$sdate = $data['sdate'].' 00:00:00';
		$edate = $data['edate'].' 23:59:59';
		$sql = "SELECT m.member_name, p.type, p.io, p.cdate, p.brief ";
		$sql.= "FROM member_point p ";				
		$sql.= "LEFT JOIN member m ON m.id = p.member_id ";		
		$sql.= "WHERE p.cdate BETWEEN '$sdate' AND '$edate' AND io<0 ";
	...
	function report_student_point_monthly($data){
		...
		for($i= strtotime($sdate); $i<= strtotime($edate); $i+=(86400*date('t',$i)) ){
			$fields[chr($ascii++)] = date('Y/m',$i);
		}
		$fields[chr($ascii++)] ='預收收入金額';
		$fields[chr($ascii++)] ='備註';
...
EOT;
		...
		if($rs2->count>0){
			while($row = $rs2->fetch()){
					...
					//手動贈點
					$sql = "SELECT SUM(io) as io FROM member_point WHERE member_id={$row['id']} AND `type` =110";
					$rs3->query($sql);
					$r3 = $rs3->fetch();
					if($r3 && isset($r3['io']))
						$row['point']+=$r3['io'];
					...
					//累計使用點數
					$sql = "SELECT SUM(io) as io, balance FROM member_point WHERE member_id={$row['id']} AND `type` in (10,100,110) GROUP BY id ORDER BY id DESC";
					$rs3->query($sql);
					$r3 = $rs3->fetch();
					$used = abs($r3['io']);
					$balance = $r3['balance'];
					...
					//剩餘點數
					$sql = "SELECT balance as io FROM member_point WHERE member_id={$row['id']} AND `type` in (10,100,110)";
					$rs3->query($sql);
					$r3 = $rs3->fetch();
					$used = abs($r3['io']);
					...
					$ascii = 73;
					$sql = <<<EOT
					SELECT DATE_FORMAT(cdate,'%Y%m') as cdate, SUM(io) as io FROM member_point WHERE member_id={$row['id']} AND 
					`type` in (10,100,110) AND 
					cdate BETWEEN '$sdate 00:00:00' AND '$edate 23:59:59'
					GROUP BY DATE_FORMAT(cdate,'%Y%m')
EOT;
					$rs3->query($sql);
					$monthly = array();
					while( $r3 = $rs3->fetch() ){
						$monthly[$r3['cdate']] = abs($r3['io']);
					}
					
					for($i= strtotime($sdate); $i<= strtotime($edate); $i+=(86400*date('t',$i)) ){
						$values[chr($ascii++)] = (int)$monthly[date('Ym',$i)];
					}
					$values[chr($ascii++)] = $duty_free - ( $used * $avg_point_price);
					$values[chr($ascii++)] ='';			
